...
このアプリでは、収集したいチャンネルを管理するためにタグを使用します。例えば、特定のトピックやプロジェクトごとに関連するチャンネルをまとめることができます。
タグには、ユーザーのみがアクセスできる「ユーザータグ」と、誰でもアクセスできる「パブリックタグ」という2つの種類があります。
「set」コマンドを使用すると、特定のタグにチャンネルを登録できます。例えば、set --publicというオプションを追加すると、パブリックなタグの管理が可能です。
