...
use super::channel_list_folder::{ChannelListFolders, FolderTag};
...
pub async fn load_ch_list_folders_json_from_path(
    path_ch_list: &Path,
) -> anyhow::Result<ChannelListFolders> {
...
pub async fn load_ch_list_folders_json() -> anyhow::Result<ChannelListFolders> {
    ...
    load_ch_list_folders_json_from_path(path_ch_list).await
}
...
pub async fn operate_channel_list(
    ...
    let folder_tag = FolderTag::new(folder.to_string()).with_owner_id(user);

    let new_ch_list_folders = match operation {
        FolderOperation::AddPrivate => ch_list_folders.add_channel_list(folder_tag, channel),
        FolderOperation::AddPublic => {
            ch_list_folders.add_channel_list(folder_tag.without_owner_id(), channel)
        }
        FolderOperation::Delete => ch_list_folders.delete_channel_list(folder_tag, &channel),
        FolderOperation::RetrieveBot => {
            let retrieve_bot_unwrap = retrieve_bot.context("retrieve bot must be bool")?;
            ch_list_folders.retrieve_bot(folder_tag, retrieve_bot_unwrap)
        }
    };

    let new_content = serde_json::to_string_pretty(new_ch_list_folders)?;
...
pub async fn get_channel_list(
    ...
    let folder_tag = FolderTag::new(tag).with_owner_id(user);
...
pub async fn get_tag_list(user_id: SlackUserId) -> anyhow::Result<Vec<String>> {
    let ch_list_folders: ChannelListFolders = load_ch_list_folders_json().await?;
    let tags = ch_list_folders.get_tag_list(&user_id);
    Ok(tags)
}
#[cfg(test)]
mod tests {
    ...
    async fn test_create_and_add_channel_list() {
        ...
        let channel_folder = load_ch_list_folders_json_from_path(path_test)
            .await
            .unwrap();
        let folder_tag = FolderTag::new("test".to_string()).with_owner_id(user_id);
    ...
    async fn delete_channel_list_test() {
        let path_test = Path::new(PATH_CH_LIST_FOLDER_TEST);
        let folder_list = load_ch_list_folders_json_from_path(path_test)
            .await
            .unwrap();
        let user_id = SlackUserId::new("U0123455".to_string());
        ...
        .await;
        let add_delete_folder_list = load_ch_list_folders_json_from_path(path_test)
            .await
            .unwrap();
