...
pub async fn load_ch_list_folders_json(user: SlackUserId) -> anyhow::Result<ChannelListFolder> {
    let ch_list_folder = load_user_folders_json()
        .await?
        .get_user_ch_list_folders(&user.0)
        .clone();
    Ok(ch_list_folder)
}
...
pub async fn operate_channel_list(
    ...
    let path_ch_list = Path::new(PATH_CH_LIST_FOLDER);
    let mut ch_list_folders = load_ch_list_folders_json(user).await?;

    let new_ch_list_folders = match operation {
        FolderOperation::AddPrivate => ch_list_folders.add_channel_list(folder, channel),
        FolderOperation::AddPublic => ch_list_folders.add_channel_list(folder, channel),
